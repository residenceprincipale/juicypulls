...
export default class ShooterManager {
	...
	endGame() {
		...
		const animateOutPromises = this._activeTargets.map((target) => target.animateOut(0.5, Math.random() * 0.3))
	...
	_handleShoot() {
		...
		if (hitTargets.length > 0) {
			...
			for (let i = 1; i < hitTargets.length; i++) {
				...
				if (targetZ > closestDistance) {
					// In Three.js, higher z values are closer to camera
